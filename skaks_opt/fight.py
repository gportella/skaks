"""Run a single UCI arena game between Skaks and a reference engine."""
...
__all__ = ["parse_args", "run_game", "main"]
...
def _resolve_engine(path_str: str) -> str:
    ...
    for cand in search:
        ...
        if not cand.suffix:
            py_variant = cand.with_suffix(".py")
            if py_variant.exists():
                return str(py_variant.resolve())
    which = shutil.which(path_str)
    if which:
        return which
    if not candidate.suffix:
        which_py = shutil.which(f"{path_str}.py")
        if which_py:
            return which_py
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run engine-vs-engine fights.")
    parser.add_argument(
        "--engine",
