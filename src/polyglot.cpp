...
static inline uint16_t decode_from(uint16_t m) { return (m >> 6) & 0x3F; }
static inline uint16_t decode_to(uint16_t m)   { return m & 0x3F; }
static inline uint16_t decode_promo(uint16_t m){ return (m >> 12) & 0x7; }
...
static inline OccupancyType promo_occ(uint16_t promo, SideToMove stm) {
  if (promo == 0) return OccupancyType::empty;
  const bool white = (stm == SideToMove::White);
  switch (promo) {
    case 1: return white ? OccupancyType::wN : OccupancyType::bN;
    case 2: return white ? OccupancyType::wB : OccupancyType::bB;
    case 3: return white ? OccupancyType::wR : OccupancyType::bR;
    case 4: return white ? OccupancyType::wQ : OccupancyType::bQ;
    default: return OccupancyType::empty;
  }
}
...
std::optional<uint32_t> decode_move(uint16_t poly_move, const Board& b) {
  ...
  uint16_t count = 0;
  auto moves = generate_legal_moves(const_cast<Board&>(b), b.side_to_move, count);
...
bool load_book(const std::string& path, Book& out) {
  ...
  while (true) {
    ...
    uint32_t learn = ((learn_be & 0x000000FFU) << 24) |
                     ((learn_be & 0x0000FF00U) << 8) |
                     ((learn_be & 0x00FF0000U) >> 8) |
                     ((learn_be & 0xFF000000U) >> 24);
...
std::span<const Entry> lookup(const Book& book, const Board& b) {
  ...
  if (it == book.end()) return std::span<const Entry>(empty);
...
std::optional<uint32_t> select(const Board& b, std::span<const Entry> entries,
  ...
  struct Cand { uint32_t move; uint16_t weight; };
  ...
  uint64_t total = 0;
  for (auto& c : cands) total += c.weight ? c.weight : 1;
