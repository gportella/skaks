...
void decode_fen(const char* fen_str, int* player, int* castle, int* fifty,
                int* move_number, int* piece, int* square) {
  /*decode fen*/
  int sq, index = 2;
  ...
  for (int r = 7; r >= 0; r--) {
    for (int f = 0; f <= 7; f++) {
      ...
      if ((pfen = strchr(piece_name, *p)) != 0) {
        const int pc = static_cast<int>(strchr(piece_name, *pfen) - piece_name);
        if (pc == 1) {
          piece[0] = pc;
          square[0] = sq;
        } else if (pc == 7) {
