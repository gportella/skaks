...
MOVE_TIME=0.20                       # Seconds per move for Skaks candidate/baseline
OPP_MOVE_TIME=0.20                   # Seconds per move allocated to Sunfish opponent
...
skaks-opt param-optimize \
  ...
  --external-opponent \
  --opponent "$OPPONENT" \
  --opponent-time-per-move "$OPP_MOVE_TIME" \
