...
static_assert(PSQTBuckets % 8 == 0,
              "Per feature PSQT values cannot be processed at granularity lower "
              "than 8 at a time.");
...
#define vec_msb_pack_16(a, b)                                                   \
  _mm_packs_epi16(_mm_srli_epi16(a, 7), _mm_srli_epi16(b, 7))
...
template <typename SIMDRegisterType, typename LaneType, int NumLanes,
          int MaxRegisters>
...
#define LaneSize sizeof(LaneType)
  ...
  // Look for the largest divisor of the ideal register count that is smaller
  // than MaxRegisters
...
private:
  ...
  static constexpr int NumPsqtRegs =
      BestRegisterCount<psqt_vec_t, PSQTWeightType, PSQTBuckets,
                        NumRegistersSIMD>();

  static constexpr IndexType TileHeight = NumRegs * sizeof(vec_t) / 2;
  static constexpr IndexType PsqtTileHeight =
      NumPsqtRegs * sizeof(psqt_vec_t) / 4;
  static_assert(HalfDimensions % TileHeight == 0,
                "TileHeight must divide HalfDimensions");
  static_assert(PSQTBuckets % PsqtTileHeight == 0,
                "PsqtTileHeight must divide PSQTBuckets");
#endif
...
public:
  ...
  bool write_parameters(std::ostream& stream) const {
    ...
    write_leb_128<PSQTWeightType>(stream, psqtWeights,
                                  PSQTBuckets * InputDimensions);
  ...
  std::int32_t transform(const PositionLike& pos, OutputType* output,
                         int bucket) const {
    update_accumulator<PositionLike, WHITE>(pos);
    update_accumulator<PositionLike, BLACK>(pos);
    ...
    for (IndexType p = 0; p < 2; ++p) {
      ...
      const vec_t* in0 =
          reinterpret_cast<const vec_t*>(&(accumulation[perspectives[p]][0]));
      const vec_t* in1 = reinterpret_cast<const vec_t*>(
          &(accumulation[perspectives[p]][HalfDimensions / 2]));
      ...
      for (IndexType j = 0; j < HalfDimensions / 2; ++j) {
        ...
        BiasType sum1 = accumulation[static_cast<int>(perspectives[p])]
                                    [j + HalfDimensions / 2];
        sum0 = std::clamp<BiasType>(sum0, 0, 127);
        sum1 = std::clamp<BiasType>(sum1, 0, 127);
        output[offset + j] =
            static_cast<OutputType>(unsigned(sum0 * sum1) / 128);
      }
  ...
  template <typename PositionLike>
  void hint_common_access(const PositionLike& pos) const {
    hint_common_access_for_perspective<PositionLike, WHITE>(pos);
    hint_common_access_for_perspective<PositionLike, BLACK>(pos);
  }

private:
  template <typename PositionLike, Color Perspective>
  [[nodiscard]] std::pair<StateInfo*, StateInfo*>
  try_find_computed_accumulator(const PositionLike& pos) const {
  ...
  // NOTE: The parameter states_to_update is an array of position states, ending
  // with nullptr.
  //       All states must be sequential, that is states_to_update[i] must either
  //       be reachable by repeatedly applying ->previous from
  //       states_to_update[i+1] or states_to_update[i] == nullptr. computed_st
  //       must be reachable by repeatedly applying ->previous on
  //       states_to_update[0], if not nullptr.
  template <typename PositionLike, Color Perspective, size_t N>
  void update_accumulator_incremental(const PositionLike& pos,
    ...
    const Square ksq = pos.template square<KING>(Perspective);
    ...
    {
      int i = N - 2; // Last potential state to update. Skip last element because
                     // it must be nullptr.
      ...
      for (; i >= 0; --i) {
        ...
        const StateInfo* end_state =
            i == 0 ? computed_st : states_to_update[i - 1];
    ...
    // Now update the accumulators listed in states_to_update[], where the last
    ...
    if (states_to_update[1] == nullptr &&
      ...
      auto accIn = reinterpret_cast<const vec_t*>(
          &(st->*accPtr).accumulation[Perspective][0]);
      ...
      auto columnPsqtA =
          reinterpret_cast<const psqt_vec_t*>(&psqtWeights[offsetPsqtA]);

      if (removed[0].size() == 1) {
        for (std::size_t k = 0;
             k < PSQTBuckets * sizeof(std::int32_t) / sizeof(psqt_vec_t); ++k)
          accPsqtOut[k] = vec_add_psqt_32(
              vec_sub_psqt_32(accPsqtIn[k], columnPsqtR0[k]), columnPsqtA[k]);
      } else {
      ...
      for (IndexType j = 0; j < PSQTBuckets / PsqtTileHeight; ++j) {
        ...
        for (IndexType i = 0; states_to_update[i]; ++i) {
          ...
          for (const auto index : removed[i]) {
            ...
            auto columnPsqt =
                reinterpret_cast<const psqt_vec_t*>(&psqtWeights[offset]);
            for (std::size_t k = 0; k < NumPsqtRegs; ++k)
  ...
  template <typename PositionLike, Color Perspective>
  void update_accumulator_refresh(const PositionLike& pos) const {
#ifdef VECTOR
    // Gcc-10.2 unnecessarily spills AVX2 registers if this array
    // is defined in the VECTOR code below, once in each branch
    vec_t acc[NumRegs];
  ...
  template <typename PositionLike, Color Perspective>
  void hint_common_access_for_perspective(const PositionLike& pos) const {
    ...
    auto [oldest_st, _] =
        try_find_computed_accumulator<PositionLike, Perspective>(pos);

    if ((oldest_st->*accPtr).computed[Perspective]) {
      // Only update current position accumulator to minimize work.
      StateInfo* states_to_update[2] = {pos.state(), nullptr};
      update_accumulator_incremental<PositionLike, Perspective, 2>(
          pos, oldest_st, states_to_update);
    } else
      update_accumulator_refresh<PositionLike, Perspective>(pos);
  }

  template <typename PositionLike, Color Perspective>
  void update_accumulator(const PositionLike& pos) const {
    ...
    if ((oldest_st->*accPtr).computed[Perspective]) {
      ...
      // Now update the accumulators listed in states_to_update[], where the last
      // element is a sentinel. Currently we update 2 accumulators.
      ...
      update_accumulator_incremental<PositionLike, Perspective, 3>(
          pos, oldest_st, states_to_update);
    } else {
      update_accumulator_refresh<PositionLike, Perspective>(pos);
    }
