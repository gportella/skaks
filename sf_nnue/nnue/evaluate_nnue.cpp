...
// Modified from Stockfish's to adapt to SkakS eval and bitboard representation
...
LargePagePtr<FeatureTransformer<TransformedFeatureDimensionsBig,
                                &StateInfo::accumulatorBig>>
    featureTransformerBig;
LargePagePtr<FeatureTransformer<TransformedFeatureDimensionsSmall,
                                &StateInfo::accumulatorSmall>>
    featureTransformerSmall;

// Evaluation function
AlignedPtr<Network<TransformedFeatureDimensionsBig, L2Big, L3Big>>
    networkBig[LayerStacks];
AlignedPtr<Network<TransformedFeatureDimensionsSmall, L2Small, L3Small>>
...
template <typename T> void initialize(AlignedPtr<T>& pointer) {

  pointer.reset(reinterpret_cast<T*>(std_aligned_alloc(alignof(T), sizeof(T))));
  std::memset(pointer.get(), 0, sizeof(T));
}

template <typename T> void initialize(LargePagePtr<T>& pointer) {
...
// Read network header
static bool read_header(std::istream& stream, std::uint32_t* hashValue,
                        std::string* desc) {
...
static bool write_header(std::ostream& stream, std::uint32_t hashValue,
...
static bool read_parameters(std::istream& stream, NetSize netSize,
  ...
  if (netSize == Big && !Detail::read_parameters(stream, *featureTransformerBig))
    return false;
  if (netSize == Small &&
      !Detail::read_parameters(stream, *featureTransformerSmall))
    return false;
  for (std::size_t i = 0; i < LayerStacks; ++i) {
...
static bool write_parameters(std::ostream& stream, NetSize netSize,
  ...
  if (!write_header(stream, HashValue[netSize], netDescription))
    return false;
  if (netSize == Big &&
      !Detail::write_parameters(stream, *featureTransformerBig))
    return false;
  if (netSize == Small &&
      !Detail::write_parameters(stream, *featureTransformerSmall))
    return false;
  for (std::size_t i = 0; i < LayerStacks; ++i) {
    if (netSize == Big && !Detail::write_parameters(stream, *(networkBig[i])))
      return false;
    if (netSize == Small &&
        !Detail::write_parameters(stream, *(networkSmall[i])))
      return false;
  }
  return bool(stream);
}
...
template <NetSize Net_Size, typename PositionLike>
Value evaluate_position(const PositionLike& pos, bool adjusted,
                        int* complexity) {
  ...
  const int bucket = (pos.template count<ALL_PIECES>() - 1) / 4;
  const auto psqt =
      Net_Size == Small
...
Value evaluate(const Position& pos, bool adjusted, int* complexity) {
  return evaluate_position<Net_Size>(pos, adjusted, complexity);
}

Value evaluate_adapter(const AdapterPosition& pos, NetSize netSize,
                       bool adjusted, int* complexity) {
  return netSize == Small ? evaluate_position<Small>(pos, adjusted, complexity)
                          : evaluate_position<Big>(pos, adjusted, complexity);
}
...
static void format_cp_aligned_dot(Value v, std::stringstream& stream) {
  ...
  const double pawns = std::abs(0.01 * 100 * v / 356);

  stream << (v < 0   ? '-'
             : v > 0 ? '+'
                     : ' ')
         << std::setiosflags(std::ios::fixed) << std::setw(6)
         << std::setprecision(2) << pawns;
}
...
std::optional<std::string> load_eval(std::istream& stream, NetSize netSize) {
  ...
  return read_parameters(stream, netSize, netDescription)
             ? std::make_optional(netDescription)
             : std::nullopt;
}
...
bool save_eval(
    const std::optional<std::string>& filename, NetSize netSize,
